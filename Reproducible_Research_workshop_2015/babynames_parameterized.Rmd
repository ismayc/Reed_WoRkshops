...
We have determined that (of the top 1000 names) in `r entered_year`, there were `r result` babies born in the US with the first name `r entered_name`.

Additionally, we can produce output conditional on results using the `ifelse` function in R:

`r ifelse(result < 5, "The name you entered was not very popular in that year.", "")`

**One more tweak**:

`r ifelse(result < 5, paste0("The name '", entered_name, "' was not very popular in ", entered_year, "."), "What a popular name!")`

### Try this!  Make sure to comment out lines 6-8 above in the Rmd file before running this chunk.

```{r try_this, eval=FALSE}
names_array <- c("Jack", "Jill", "Tom", "Mary")
years_array <- rep(1985, length(names_array))
